...
1. Фильтрация*:
    1. Имени автора (кто создал)
    1. Label'у (вам нужно проанализировать механику и подобрать нужный метод самостоятельно, но можете посмотреть подсказку)
    1. Assignee (на кого назначено)
...
Примечание*: фильтрация - это операция возврата только тех объектов, которые удовлетворяют заданному условию. В рамках стандартной библиотеки Java для этого существует специальный интерфейс `Predicate`. Попробуйте по аналогии с `Comporator` реализовать метод `filterBy` в менеджере, в который передавать объект, удовлетворяющий интерфейсу `Predicate`.
...
Один из ключевых навыков, которые мы будем отрабатывать в рамках данного ДЗ - умение читать JavaDoc и примеры из него.

Почему именно так: потому что не всегда для вас будут написаны готовые статьи, книжки и т.д., достаточно часто в лучшем варианте у вас будет JavaDoc с примерами, в варианте похуже - просто исходники (в худшем - просто бинарные файлы без документации).
